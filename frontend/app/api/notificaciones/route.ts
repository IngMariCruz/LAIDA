...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, tipo, titulo, mensaje, lead_id, bot_id, from } = body
    ...
    // Manager: solicitar al super_admin ejecutar campañas pendientes
    if (action === "request_run_campaigns") {
      if (user.rol !== "manager") {
        return NextResponse.json(
          { error: "Solo un manager puede solicitar la ejecución" },
          { status: 403 }
        )
      }

      const superAdmins = getAllUsuarios().filter((u) => u.rol === "super_admin")
      if (superAdmins.length === 0) {
        return NextResponse.json(
          { error: "No hay super admins para notificar" },
          { status: 400 }
        )
      }

      const requester = typeof from === "string" && from.trim() ? from.trim() : (user.nombre || user.correo)
      superAdmins.forEach((sa) => {
        createNotificacion({
          usuario_id: sa.id,
          tipo: "sistema",
          titulo: "Solicitud de ejecución de campañas",
          mensaje: `${requester} solicitó ejecutar campañas pendientes.`,
          lead_id: null,
          bot_id: null,
        })
      })

      return NextResponse.json({ success: true, notified: superAdmins.length })
    }
