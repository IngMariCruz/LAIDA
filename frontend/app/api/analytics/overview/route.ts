...
export async function GET(request: NextRequest) {
  try {
    ...
    // leads last 7 days
    const dayWhere = botFilter ? `${botFilter} AND created_at >= DATE('now','-7 days')` : `WHERE created_at >= DATE('now','-7 days')`
    ...
    // popular products
    const productJoinFilter = botFilterAliasL
      ? ` AND l.bot_id IN (${params.map(() => '?').join(',')})`
      : ''

    const prodStmt = db.prepare(`
      SELECT p.nombre, COUNT(l.id) as count
      FROM productos p
      LEFT JOIN leads l ON l.producto_id = p.id${productJoinFilter}
      GROUP BY p.id
      ORDER BY count DESC
      LIMIT 10
    `)
